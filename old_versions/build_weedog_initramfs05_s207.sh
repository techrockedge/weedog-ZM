#!/bin/sh
# Build_weedog_initramfs05_s207 to create WeeDog Linux initramfs Model05
# (s207 means [s]witch_root revision 2.0.7) 
...
kernel="$1"
case "$1" in
	'-v'|'--version') echo "Build WeeDog initramfs05 [s]witch_root Revision 2.0.7";exit;;
	'-h'|'--help'|'-?') printf '
Usage:
...
initramfs. If you wish to remove any of these kernels (using for
...
of the kernel you wish to use with your initramfs
...
# Revision 2.0.7  Date: 12 Oct 2019
...
# usbwait  # usbwait for slow devices. If empty string (or omitted) then auto-seek till timeout
...
# process any grub linux/kernel line rdshN argument or "debug" instruction for source plugin or debug sh
_rdsh (){
	[ -s "${mountfrom}"/${1}.plug ] && . "${mountfrom}"/${1}.plug
	if grep -q $1 /proc/cmdline || [ "$2" == "debug" ]; then
			# Start a busybox job control debug shell at initramfs/init rdsh break point
...
case "$kernel" in
	void)
		...
		cp -af firstrib_rootfs/usr/lib/modules/"${kernel_version}" firstrib_rootfs_for_initramfs_sNNN/usr/lib/modules
		cp -a "${kernel1}" .
	...
	ubuntu|debian|devuan)
		...
		echo "Copying Void Linux modules to initramfs build. Please wait patiently..."
		cp -af firstrib_rootfs/lib/modules/"${kernel_version}" firstrib_rootfs_for_initramfs_sNNN/usr/lib/modules
		cp -a "${kernel1}" .
...
echo -e "\e[33mSeeking $bootpartition ... Delay will be ${usbwait:-variable up to timeout of 30} seconds ...\e[0m" >/dev/console
mkdir -p /mnt/${bootpartition}
c=0  # timeout counter for seek bootpartition
if [ "$usbwait" ]; then
	sleep $usbwait
...
[ $c -gt 30 ] && _rdsh seek_timeout debug

_rdsh rdsh0  # Source rdsh0.plug if it exists. Thereafter, if grub kernel-line rdsh0 specified start busybox debug shell
_rdsh inram00  # Source inram00 plugin, for example to set up swap space or zram
...
if	[ $copy2ram -eq 0 ]; then
	...
	cp -a pre_switch_root.plug "${mountfrom}" 2>/dev/null
	cp -a inram00.plug "${mountfrom}" 2>/dev/null  # probably not needed here but ok
...
_rdsh rdsh1  # Source rdsh1.plug if it exists. Thereafter, if grub kernel-line rdsh0 specified start busybox debug shell
...
_rdsh rdsh2  # Source rdsh2.plug if it exists. Thereafter, if grub kernel-line rdsh2 specified start busybox debug shell
...
_rdsh rdsh3  # Source rdsh3.plug if it exists. Thereafter, if grub kernel-line rdsh3 specified start busybox debug shell
...
_rdsh rdsh4  # Source rdsh4.plug if it exists. Thereafter, if grub kernel-line rdsh4 specified start busybox debug shell
...
# If grub kernel-line rdsh5 specified then start busybox debug shell
_rdsh rdsh5
